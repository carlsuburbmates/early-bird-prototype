...
const STATE_TONE: Record<RequestState, string> = {
  ...
  provider_invited: "bg-[color:var(--color-status-progress)]/15 text-[color:var(--color-status-progress)]",
  awaiting_response: "bg-[color:var(--color-status-progress)]/15 text-[color:var(--color-status-progress)]",
  accepted: "bg-[color:var(--color-status-success)]/15 text-[color:var(--color-status-success)]",
  awaiting_payment: "bg-[color:var(--color-status-warning)]/15 text-[color:var(--color-status-warning)]",
  details_released: "bg-[color:var(--color-status-success)]/15 text-[color:var(--color-status-success)]",
...
export function StateBadge({ state, className }: { state: RequestState; className?: string }) {
  return (
    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium", STATE_TONE[state], className)}>
...
export function InvitationBadge({
  ...
  return (
    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize", tone[state])}>
      {state.replace("_", " ")}
    </span>
  );
}
