...
import { formatDate, formatDateTime, formatMoney, relativeFromNow } from "@/mock/utils";
import { SERVICE_LABELS } from "@/mock/types";
import { Lock, Mail, Phone, MapPin, CalendarDays, CheckCircle2 } from "lucide-react";
...
export function ProviderDashboard() {
  ...
  const acceptedAwaitingFee = myInvites
    .filter((i) => i.state === "accepted")
    .map((i) => ({ inv: i, fee: fees.find((f) => f.invitationId === i.id && f.status !== "paid") }))
    .filter((x) => x.fee);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {isDemoMode() && (
          <div className="w-64">
            ...
            <p className="mt-1 text-[10px] text-muted-foreground">Demo only — switch provider context</p>
          </div>
        )}
      </div>
