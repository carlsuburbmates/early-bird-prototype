...
import { Activity, AlertTriangle, CheckCircle2, Clock, Inbox, Users, Workflow, XCircle } from "lucide-react";
...
function HealthPage() {
  ...
  const cards: { Icon: LucideIcon; label: string; value: number; tone?: "warning" | "critical" }[] = [
    ...
    { Icon: AlertTriangle, label: "Open exceptions", value: metrics.openExceptions, tone: metrics.openExceptions > 0 ? "warning" : undefined },
    { Icon: AlertTriangle, label: "Critical exceptions", value: metrics.criticalExceptions, tone: metrics.criticalExceptions > 0 ? "critical" : undefined },
    { Icon: Workflow, label: "Scheduled jobs", value: metrics.scheduledJobs },
    { Icon: Workflow, label: "Failed jobs", value: metrics.failedJobs, tone: metrics.failedJobs > 0 ? "critical" : undefined },
    { Icon: Clock, label: "SLA breaches", value: metrics.slaBreaches, tone: metrics.slaBreaches > 0 ? "warning" : undefined },
    { Icon: Activity, label: "Overdue fees", value: metrics.overdueFees, tone: metrics.overdueFees > 0 ? "critical" : undefined },
  ...
  return (
    <div className="space-y-6">
        ...
        {breaches.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-[color:var(--color-console-muted)]/40 text-left text-xs uppercase text-[color:var(--color-console-muted-foreground)]">
              <tr><th className="px-3 py-2">Request</th><th className="px-3 py-2">SLA due</th><th className="px-3 py-2">Overdue</th><th className="px-3 py-2">Next action</th></tr>
            </thead>
            <tbody>
              {breaches.map(({ request, overdueMins }) => (
                <tr key={request.id} className="border-t border-[color:var(--color-console-border)]">
                  <td className="px-3 py-2 font-mono text-xs">
                    <Link to="/ops/requests/$id" params={{ id: request.id }} className="hover:underline">{request.id}</Link>
                  </td>
                  <td className="px-3 py-2 text-xs">{request.slaDueAt ? formatDateTime(request.slaDueAt) : "—"}</td>
                  <td className="px-3 py-2 text-xs text-[color:var(--color-status-critical)]">{Math.round(overdueMins / 60)}h</td>
                  <td className="px-3 py-2 text-xs">{request.nextAction ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {providerMetrics.map((m) => (
              <tr key={m.provider.id} className="border-t border-[color:var(--color-console-border)]">
      ...
      <p className="text-xs text-[color:var(--color-console-muted-foreground)]">Snapshot at {formatDateTime(now)}</p>
    </div>
  );
}

function Card({ Icon, label, value, tone }: { Icon: LucideIcon; label: string; value: number; tone?: "warning" | "critical" }) {
  const toneClass =
    tone === "critical"
