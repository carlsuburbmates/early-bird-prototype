  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard Icon={Inbox} label="Active requests" value={active.length} />
        <StatCard Icon={AlertTriangle} label="Open exceptions" value={openExc.length} tone={openExc.length > 0 ? "warning" : undefined} />
        <StatCard Icon={Workflow} label="Scheduled jobs" value={scheduled.length} />
        <StatCard Icon={Activity} label="Audit (24h)" value={recent} />
      </div>

      <Panel title="Active requests" right={<Link to="/ops/requests" className="text-xs text-[color:var(--color-console-accent)] hover:underline">View all →</Link>}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[color:var(--color-console-muted)]/40 text-left text-xs uppercase text-[color:var(--color-console-muted-foreground)]">
              <tr><th className="px-3 py-2">Request</th><th className="px-3 py-2">State</th><th className="px-3 py-2">Next action</th><th className="px-3 py-2">SLA</th></tr>
            </thead>
            <tbody>
              {active.slice(0, 10).map((r) => (
                <tr key={r.id} className="border-t border-[color:var(--color-console-border)] hover:bg-[color:var(--color-console-muted)]/30">
                  ...
                  <td className="px-3 py-2 text-[color:var(--color-console-muted-foreground)]">{r.nextAction ?? "—"}</td>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel title="Open exceptions">
          {openExc.length === 0 ? <Empty>No open exceptions.</Empty> :
            <ul className="divide-y divide-[color:var(--color-console-border)]">
              {openExc.map((e) => (
                <li key={e.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  <span><Link to="/ops/requests/$id" params={{ id: e.requestId }} className="font-mono text-xs hover:underline">{e.requestId}</Link> · {e.type.replace(/_/g, " ")}</span>
                  <span className="text-xs text-muted-foreground">{relativeFromNow(e.openedAt, now)}</span>
                </li>
              ))}
            </ul>}
        </Panel>
            ...
            <ul className="divide-y divide-[color:var(--color-console-border)]">
              {scheduled.slice(0, 8).map((a) => (
                <li key={a.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  <span>{a.type.replace(/_/g, " ")} <span className="ml-1 font-mono text-[10px] text-muted-foreground">{a.requestId}</span></span>
                  <span className="text-xs text-muted-foreground">{formatDateTime(a.scheduledFor)}</span>
                </li>
              ))}
            </ul>}
        </Panel>
...
function StatCard({ Icon, label, value, tone }: { Icon: LucideIcon; label: string; value: number; tone?: "warning" }) {
  return (
    <div className="rounded-xl border border-[color:var(--color-console-border)] bg-[color:var(--color-console-surface)] p-4">
      <div className="flex items-center gap-2 text-xs text-[color:var(--color-console-muted-foreground)]"><Icon className="h-3.5 w-3.5" /> {label}</div>
      <div className={"mt-2 text-2xl font-semibold " + (tone === "warning" ? "text-[color:var(--color-status-critical)]" : "")}>{value}</div>
    </div>
  );
}
