          ...
          <tbody>
            {exceptions.map((e) => (
              <tr key={e.id} className="border-t border-[color:var(--color-console-border)]">
                ...
                <td className="px-3 py-2 font-mono text-xs">{e.cartItemId ?? "—"}</td>
                <td className="px-3 py-2 font-mono text-xs">{e.serviceProductId ?? "—"}</td>
                ...
                <td className="px-3 py-2 text-xs">
                  {e.resolvedAt ? `Resolved ${relativeFromNow(e.resolvedAt, now)}` : "Open"}
                </td>
